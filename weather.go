...
var api *slack.Client = slack.New(os.Getenv("MITSUHA_BOT_TOKEN"))
...
func WhetherForecast() (forecast_today string ,forecast_tomorrow string) {
	...
	response.Header.Add("x-rapidapi-key", os.Getenv("WEATHER_API_KEY"))
...
func Reglarsend(){
    ...
    night := time.Date(2021, 5, 1, 20, 00, 0, 0, time.Local)//夜の投稿時間設定
